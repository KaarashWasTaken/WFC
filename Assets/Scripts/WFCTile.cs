...
public class WFC2Tile : MonoBehaviour
{
	//THIS TILE'S TYPE
    public TileType tileType;
	//LIST OF ALL SPRITES
    ...
    public List<bool> openDirections = new() { false, false, false, false };
	//
    public WFC2Tile[] adjacentTiles = new WFC2Tile[4];
    public TileManager manager;
    public Vector2 tileLoc;

    public int rotation = 0;
	//ALLOWED NEIGHBOURS BASED ON TYPE AND ROTATION
    public Dictionary<TileType, List<int>>[] allowedNeighbors = new Dictionary<TileType, List<int>>[4];
    ...
    public void CollapseTile()
    {
        ...
        tileType = possibleTileTypes[Random.Range(0, possibleTileTypes.Count)];
        rotation = GetPossibleRotations(tileType)[Random.Range(0, GetPossibleRotations(tileType).Count)];
        UpdateSprite();
    ...
    List<int> GetPossibleRotations(TileType type)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                if (adjacentTiles[i].allowedNeighbors[(i + 2) % 4].ContainsKey(type))
                {
                    List<int> allowed = adjacentTiles[i].allowedNeighbors[(i + 2) % 4][type];
    ...
    public void UpdatePossibleTileTypes(WFC2Tile neighbor, int direction)
    {
        List<TileType> newPossibleTypes = new List<TileType>(possibleTileTypes);
        foreach (TileType type in possibleTileTypes)
        {
            if (!allowedNeighbors[direction].ContainsKey(type))
            {
            ...
            else
            {
                ...
                List<int> allowedRotations = allowedNeighbors[direction][type];
    ...
    public void ResetTile()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            allowedNeighbors[i] = new Dictionary<TileType, List<int>>();
            for (int j = 0; j < (int)TileType.LASTINLIST; j++)
            {
                allowedNeighbors[i].Add((TileType)j, new List<int>() { 0, 90, 180, 270 });
            }
        }
